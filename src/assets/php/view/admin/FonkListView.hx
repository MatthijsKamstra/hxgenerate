...
class FonkListView extends BaseView
{
	...
	{
		var hashMap : Map<String, HermanBroodVO> = new Map();
		
		if (!sys.FileSystem.exists("_data")) sys.FileSystem.createDirectory("_data");
		if(sys.FileSystem.exists(AppConstants.FONK_LIST_JSON )){
			var __arr : Array<HermanBroodVO> = HermanBroodVO.parse(sys.io.File.getContent(AppConstants.FONK_LIST_JSON )); 
		...
		var str = sys.io.File.getContent('https://slack.com/api/users.list?token='+AppConstants.SLACK_TOKEN+'&presence=1&pretty=1');
		...
		var f:FileOutput = File.write(AppConstants.FONK_LIST_JSON ,false);
